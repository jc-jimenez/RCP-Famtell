...
export default function CreditosClient({ account }: { account: Account }) {
  ...
  const balancePct = Math.min(100, (account.credits_balance / (account.subscription_plan === 'pro' ? 500 : account.subscription_plan === 'enterprise' ? 2000 : 100)) * 100)
  const barColor = balancePct > 40 ? 'bg-emerald-500' : balancePct > 15 ? 'bg-amber-500' : 'bg-red-500'
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-white">Créditos y plan</h1>
        <p className="text-slate-400 text-sm mt-0.5">{account.company_name}</p>
      </div>
      ...
      <div className="card p-6">
        <div className="flex items-end justify-between mb-3">
          <div>
            <p className="text-sm text-slate-400 font-medium">Créditos disponibles</p>
            <p className="text-4xl font-bold text-white mt-1">{account.credits_balance}</p>
          </div>
          <div className="text-right">
            <span className={`text-xs font-semibold px-3 py-1 rounded-full border ${
              account.subscription_plan === 'pro' ? 'text-purple-300 bg-purple-950/60 border-purple-700'
              : account.subscription_plan === 'enterprise' ? 'text-amber-300 bg-amber-950/60 border-amber-700'
      ...
      <div className="card p-5">
        <h2 className="text-sm font-semibold text-slate-200 mb-3">Costo por acción</h2>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {Object.entries(MODULE_COSTS).map(([action, cost]) => (
            <div key={action} className="flex items-center justify-between bg-slate-800/50 rounded-lg px-3 py-2">
              ...
              <span className="text-xs font-mono font-bold text-white">{cost}cr</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Planes */}
      <div>
        <h2 className="text-sm font-semibold text-slate-200 mb-4">Planes disponibles</h2>
          ...
          {PLANS.map((plan) => {
            ...
            return (
              <div key={plan.id} className={`card p-5 flex flex-col border transition-colors ${
                isCurrent ? 'border-role-consultor bg-purple-950/10' : 'border-slate-800'
              }`}>
                <div className="flex items-center justify-between mb-3">
                  <p className="font-bold text-white">{plan.name}</p>
                  {isCurrent && <span className="text-xs text-purple-300 bg-purple-950/60 border border-purple-800 px-2 py-0.5 rounded-full">Actual</span>}
                </div>
                <p className="text-2xl font-bold text-white mb-1">{plan.price}</p>
                <p className="text-xs text-slate-500 mb-4">{plan.credits} créditos/mes incluidos</p>
                <ul className="space-y-1 flex-1 mb-5">
                  {plan.features.map(f => (
                    <li key={f} className="text-xs text-slate-400 flex gap-1.5">
                ...
                <button
                  ...
                  className={`w-full text-sm font-semibold py-2.5 rounded-xl border transition-colors disabled:opacity-50 ${
                    isCurrent
                      ...
                      : 'border-role-consultor text-purple-300 hover:bg-purple-950/40'
                  }`}
                >
