...
export async function POST(req: Request) {
  ...
  const { email, companyName, fullName, whatsapp, plan = 'starter', credits = 100 } = await req.json()
  if (!email || !companyName) return NextResponse.json({ error: 'email y companyName requeridos' }, { status: 400 })
  if (!fullName?.trim()) return NextResponse.json({ error: 'El nombre completo es obligatorio' }, { status: 400 })
...
export async function PATCH(req: Request) {
  ...
  const { accountId, status, credits, plan, fullName, whatsapp, companyName } = await req.json()
