...
**✅ Check-in semanal ejercitado end-to-end (2026-07-08):** el flujo de captura (`api/checkins`) funciona correctamente — validado con datos reales del caso Famtell: formulario → guardado en `check_ins` → análisis de Nova generado en tiempo real (específico, referencia los datos reales enviados, no genérico) → créditos descontados (3, `CREDIT_COSTS.WEEKLY_CHECKIN`) → reenviar la misma semana actualiza la fila existente (`onConflict: case_id,week_number`), no duplica.

**🔴 Gap real encontrado al ejercitarlo: no existe ninguna pantalla para VER los check-ins ya capturados.** `checkin/page.tsx` es un componente cliente sin verificación de rol (usa `role="director"` fijo en el `AppShell`, sin importar quién esté realmente autenticado) que solo renderiza el formulario de captura — no lee `check_ins` en ningún momento. Esto significa: (1) el propio directivo que ya capturó su semana no puede volver a ver lo que reportó ni el análisis de Nova pasado esa sesión; (2) el consultor tiene un tab "Check-in" en `CasoTabs.tsx` que apunta a esta misma pantalla — pero le muestra el formulario de captura, no un resumen de lo que los directivos han reportado semana a semana. El dato se guarda correctamente y persiste, simplemente no hay ninguna vista en todo el producto para consultarlo. No construido en esta sesión — el usuario decide si se prioriza.
