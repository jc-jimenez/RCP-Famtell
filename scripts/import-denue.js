...
async function main() {
  ...
  for await (const line of rl) {
    ...
    const empresa = {
      ...
      estrato:             row['per_ocu'] || row['estrato'] || '',   // per_ocu = personal ocupado/estrato
      tipo_vialidad:       row['tipo_vial'] || '',
      nombre_vialidad:     row['nom_vial'] || '',
      numero_exterior:     row['numero_ext'] || row['num_ext'] || '',
      nombre_asentamiento: row['nomb_asent'] || row['nom_asen'] || '',
      municipio:           row['municipio'] || row['nommun'] || '',
      entidad:             row['entidad'] || row['nomest'] || '',
      estado_code:         estadoCode,
      codigo_postal:       row['cod_postal'] || row['cp'] || '',
